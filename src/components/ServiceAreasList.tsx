...
export default function ServiceAreasList({ locations }: { locations: Location[] }) {
    ...
    const visibleParents = displayedParents.slice(0, visibleCount);
        ...
        <div className="flex flex-col gap-8">
            ...
            {displayedParents.length > 0 ? (
                <>
                    ...
                    {/* Load More Button */}
                    {visibleCount < displayedParents.length && (
                        <div className="flex justify-center mt-12">
                            <button
                                onClick={handleLoadMore}
                                className="group flex items-center gap-2 bg-white text-gray-900 px-8 py-4 rounded-full font-bold shadow-lg hover:shadow-xl hover:text-tapoutsPurple transition-all border border-gray-100"
                            >
                                Load More Locations
                                <ArrowRight className="w-5 h-5 group-hover:translate-y-1 transition-transform group-hover:rotate-90" />
                            </button>
                        </div>
                    )}
                </>
